...
// Cache in-memory de 10 min por instancia para reducir presión sobre el chip.
const _labelsCache = new Map();
async function findLabels(instanceName, opts = {}) {
  const ttl = opts.ttlMs || 10 * 60 * 1000; // 10 min default
  const cached = _labelsCache.get(instanceName);
  if (cached && Date.now() - cached.at < ttl) return cached.data;
  try {
    const { data } = await client.get(`/label/findLabels/${instanceName}`);
    const labels = Array.isArray(data) ? data : (data?.labels || []);
    _labelsCache.set(instanceName, { at: Date.now(), data: labels });
    return labels;
  } catch (err) {
    return cached?.data || [];
  }
}
function invalidateLabelsCache(instanceName) {
  _labelsCache.delete(instanceName);
}
